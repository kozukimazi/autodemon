...
for i in range(len(J0s)):
    for j in range(len(g0s)):
        ...
        Uf0 = 500
        Elf = Erf = 0
        Delta,g,theta =energy(Elf,Erf,g0s[j])
        Eup = (Elf+Erf)/2 + np.sqrt(Delta**2 + g**2)
        Emin = (Elf+Erf)/2 - np.sqrt(Delta**2 + g**2)
        dup,dupdag,dmin,dmindag = twolevel(Elf,Erf,g)
        omegac = 1E-2
        Ls0 = Dissipator(Elf,Erf,g,Ed0f,U00,Uf0,ev/2,-ev/2,mud0,betal,betar,betad,betaph,gl,glU,gr,grU,gd,J0s[i],omegac)
        ...
        Dph0 = Dphonon(Elf,Erf,g,betaph,J0s[i],omegac)
        Ll0 = Laux
        Lr0 = Raux
        Ld0 = DD
