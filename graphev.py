...
eVs, entropf, Slr,cohes, concv, Nls,Nds,coheveig,Qlr,Elr,Flr,Tisl,Wt,Eds,Fd,Tid,Wdf,Qdf,Id = data["eVs"], data["entropf"], data["Slr"], data["cohes"], data["concv"], data["Nls"], data["Nds"], data["coheveig"],data["Qlr"], data["Elr"], data["Flr"], data["Tisl"], data["Wt"], data["Eds"], data["Fd"], data["Tid"], data["Wdf"], data["Qdf"], data["Id"]
...
ax11.plot(eVs,Nlssec2,color='red',lw=LINE_W, label = r'$g/\kappa_{L}= 10^{2}$')
#ax11.plot(eVs,Nlssec2,color='red',lw=LINE_W, label = r'$g_{L}/\kappa_{L}= 10^{3}$')
ax11.plot(eVs,Nds, color='black',linestyle = '--',lw=LINE_W)
#(0.7,0.48)
...
ax11.set_ylabel(r'$\dot{N}_{B_L}/\kappa_{L}$',fontsize = LABEL_FS)
...
ax21.plot(eVs,coheveig, color = 'black',lw = LINE_W)
ax21.plot(eVssec,coheveigsec2, color = 'red',lw = LINE_W)    
...
ax21.set_ylabel(r'$\mathcal{C}_{l_1}(\mathrm{eig})$',fontsize = LABEL_FS)
...
#ax20.plot(eVs,Wtsec2,label = r'$\dot{W}_{2}$', color = 'm',linestyle='--',lw=LINE_W)
ax10.plot(eVs,Qlr, color='m',lw = LINE_W,label = r'$\dot{Q}_{2}$')    
ax10.plot(eVs,Qlrsec2, color='m',linestyle='--',lw = LINE_W)
#ax10.xticks(fontsize=17)  
#ax10.yticks(fontsize=17)
...
#ax20.plot(eVs,Tidsec,label = r'$T_{B_D}\dot{I}_{1}$', linestyle='--', dashes=(5, 3),color = 'r',lw=LINE_W)
...
ax10.set_ylabel(r'$\dot{N}_{B_L}/\kappa_{L}$',fontsize = LABEL_FS)
